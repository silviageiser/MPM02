...
**Data Visualisation and Linear regression**
...
Including the Type-Living_space interaction 
```{r}
lm.interact <- lm(log(Price) ~ Living_space * Type) # lm(y ~ x1 * x2) equivalent to lm(y ~ x1 + x2 + x1:x2)
summary(lm.interact)
...
data1 <- data
data1$log.price <- log(Price)
data1$log.living <- log(Living_space)
data1$log.rooms <- log(Rooms)
head(data1)
```
...
#display predictions
plot(log(Price) ~ log(Living_space), data = data1, main = 'Prediction with Model log(Price) ~ log(Living_space)', col = 'navy', pch = 16)
points(x = test$log.living, y= pred.new.living, col = 'red', pch = 16, cex = 1.5)
abline(lm.train, col = 'yellow', lwd = 2.5)


plot(log(Price) ~ log(Living_space), data = train, main = 'Prediction with Model log(Price) ~ log(Living_space)', col = 'navy', pch = 16)
