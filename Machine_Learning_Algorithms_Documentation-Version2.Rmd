...
```{r}
#Interpretation the coefficients of a Poisson model the inverse, the exponential, is needed due to use link function (log() function)
exp(coef(glm.rooms)) 
...
library(tidyverse)
filter(data, Type == "")
fitted.room <- fitted(glm.rooms)[350]
fitted.room

specific.room <- data[350,]
specific.room$Type <- "Bungalow"
specific.room
...
fitted.room + exp(coef(glm.rooms)["TypeBungalow"])

```
This house, according to the model, is expected to have 9.05 Rooms
...
knitr::include_graphics("Rplot.pdf")
```
