...
## Testing categorical variable (Furnishing_quality)and comparing by F-test (Soll das  hier noch rein ??)
```{r, include=FALSE}
...
```{r}
data <- fread('german_housing_cleaned.csv',header =T, encoding='UTF-8')
data$log.price <- log(data$Price)
data$log.living <- log(data$Living_space)
data$log.rooms <- log(data$Rooms)
data$log.lot <- log(data$Lot)
data$Condition <- factor(data$Condition)

house <- data %>% filter(Type == "Multiple dwelling" | Type == "Villa")
...
model <- train(is.multi ~ Price + Living_space,
               data = train, method = 'svmLinear2', trControl = ctrl)
print(model)
print(predict(model, newdata = test))

house1 <- expand.grid(Price = seq(min(house$Price),max(house$Price), 1000000),Living_space = seq(min(house$Living_space), max(house$Living_space),1))

house1$is.multi <- predict(model, newdata = house1)

g <- ggplot(mapping = aes(Price, Living_space)) +
