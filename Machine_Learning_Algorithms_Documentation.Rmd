...
svm_model<- 
    svm(Rooms ~ log(Price), data = trainset, type = "C-classification", 
...
mean(pred_train == trainset$Rooms)

#compute test accuracy
pred_test <- predict(svm_model, testset)
mean(pred_test == testset$Rooms)
```

```{r}

svm_model2<- 
    svm(factor(Energy_efficiency_class) ~ log(Price), data = trainset, type = "C-classification", 
        kernel = "polynomial", cost = 0.01, degree = 2)

svm_model2

#measure training and test accuracy
pred_train2 <- predict(svm_model2, trainset)
mean(pred_train2 == trainset$Energy_efficiency_class)
pred_test2 <- predict(svm_model2, testset)
mean(pred_test2 == testset$Energy_efficiency_class)
...
scatter_plot <- ggplot(data = trainset, aes(x = log(Price), y = Energy_efficiency_class, color = Energy_efficiency_class)) + 
    geom_point() + 
    scale_color_manual(values = c("red", "blue"))
 
#add plot layer marking out the support vectors 
layered_plot <- 
    scatter_plot + geom_point(data = trainset[svm_model$index, ], aes(x = log(Price), y = Energy_efficiency_class), color = "purple", size = 4, alpha = 0.5)
