...
#head(data)
data_new <- read.csv('germany_housing_cleaned_v02.csv',header =T, sep=';', encoding='UTF-8')
head(data_new)
...
d <- density(data_new$X.U.FEFF.Price)
plot(d)
```

``` {r}

d.log <- density(log(data_new$X.U.FEFF.Price))
plot(d.log)
```
